...
export class TypeModel {
    ...
    private static async fetchFields(id: number): Promise<TypeField[]> {
        return (await this.database.TYPE_FIELD_GET_TYPEID.execute(id)).map((row: any) => {
    ...
    private static async fetchType(id: number): Promise<Type> {
        const types = await this.database.TYPE_GET_ID.execute(id);
        if (types.length === 0) {
    ...
    static async exists(id: number): Promise<boolean> {
        ...
        return (await this.database.TYPE_EXISTS_ID.execute(id)).length > 0;
    }
    ...
    static async get(id: number): Promise<Type> {
        ...
        // Check cache
        let type: Type = await this.cache.get(key);
        if (type === undefined) {
            // Fetch type
            type = await this.fetchType(id);
            await this.cache.set(key, type);
        } else {
            // Reset ttl after get
            this.cache.ttl(key);
        }
    ...
    static async create(type: Type): Promise<Type> {
        ...
        let result: Type;
        await this.database.beginTransaction(async function(connection) {
            const id = (await this.database.TYPE_CREATE.executeConnection(connection, [1, type.name])).insertId;
            ...
            const fieldIds = await Promise.all(type.fields.map(function(field: TypeField) {
                ...
                return this.database.TYPE_FIELD_CREATE.executeConnection(connection, [ id, field.name, field.type, field.required, field.unique, reference ]);
            }));
            ...
            await this.database.ITEM_TABLE_CREATE.execute(result);
        });
    ...
    static async update(id: number, type: Type): Promise<Type> {
        ...
        await this.database.beginTransaction(async function(connection) {
            // Update type table
            await this.database.TYPE_UPDATE.executeConnection(connection, [1, type.name, id]);
            ...
            for (const oldField of old.fields) {
                for (let i = 0; i < fields.length; i++) {
                    ...
                    if (oldField.id === field.id) {
                        ...
                        if (oldField.type === TypeFieldType.reference && field.type !== TypeFieldType.reference) {
                            await this.database.ITEM_TABLE_FK_DROP.executeConnection(connection, oldField);
                            update = true;
                        }
                        // If type is still reference but the referenced type changed -> update (delete, create) foreign key
                            ...
                            await this.database.ITEM_TABLE_FK_DROP.executeConnection(connection, oldField);
                            await this.database.ITEM_TABLE_FK_CREATE.executeConnection(connection, field);
                            update = true;
                        }

                        // Update type or required
                        if (oldField.type !== field.type || oldField.required !== field.required) {
                            await this.database.ITEM_TABLE_FIELD_EDIT.executeConnection(connection, field);
                            update = true;
                        }
                        ...
                        if (oldField.unique && !field.unique) {
                            await this.database.ITEM_TABLE_UI_DROP.executeConnection(connection, oldField);
                            update = true;
                        } else if (!oldField.unique && field.unique) {
                            await this.database.ITEM_TABLE_UI_CREATE.executeConnection(connection, oldField);
                            update = true;
                        }
                        ...
                        if (update) {
                            const reference = field.type === TypeFieldType.reference ? field.referenceId : null;
                            await this.database.TYPE_FIELD_EDIT.executeConnection(connection,
                ...
                await this.database.TYPE_FIELD_DELETE.executeConnection(connection, oldField.id);
                await this.database.ITEM_TABLE_FIELD_DROP.executeConnection(connection, oldField);
            }

            for (const field of fields) {
                // Create type field for field id
                const reference = field.type === TypeFieldType.reference ? field.referenceId : null;
                field.id = (await this.database.TYPE_FIELD_CREATE.executeConnection(connection,
                    [ id, field.name, field.type, field.required, field.unique, reference ])).insertId;
                field.typeId = id;
                // Create new column in item table + constraints
                await this.database.ITEM_TABLE_FIELD_CREATE.executeConnection(connection, field);
                if (field.unique) {
                    await this.database.ITEM_TABLE_UI_CREATE.executeConnection(connection, field);
                }
                if (field.type === TypeFieldType.reference) {
                    await this.database.ITEM_TABLE_FK_CREATE.executeConnection(connection, field);
                }
        ...
        type = await this.fetchType(id);
        await this.cache.set(id.toString(), type);
        return type;
    }
    ...
    static async delete(id: number): Promise<void> {
        ...
        const references: TypeField[] = await this.database.TYPE_FIELD_GET_REFERENCEID.execute(id);

        await this.database.beginTransaction(async function(connection) {
            // Delete all foreign-keys and fields that reference this table
            for (const reference of references) {
                await this.database.ITEM_TABLE_FK_DROP.executeConnection(connection, reference);
                await this.database.ITEM_TABLE_FIELD_DROP.executeConnection(connection, reference);
            }

            // Drop the table and delete the type
            await this.database.ITEM_TABLE_DROP.executeConnection(connection, id);
            await this.database.TYPE_DELETE.executeConnection(connection, id);
        });
