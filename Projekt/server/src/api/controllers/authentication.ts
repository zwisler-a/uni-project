...
export function validateJsonWebToken(req: Request, res: Response, next: NextFunction) {
    ...
    jsonwebtoken.verify(groups[1], req.app.get('secret'), (err: any, decoded: object) => {
        if (err) {
            next(new OldApiError('Unauthorized', 'The provided jsonwebtoken is invalid', 401, err));
        } else {
...
export async function authenticate(req: Request, res: Response, next: NextFunction) {
    if (!('username' in req.body) || !('password' in req.body)) {
        next(new OldApiError('Bad Request', 'Required field: username, password', 400));
        return;
    }
    ...
    try {
        ...
        if (!user) {
            next(new OldApiError('Unauthorized', 'Authentication failed due to invalid credentials', 401));
            return;
        }
        ...
        next(new OldApiError('Unauthorized', 'Authentication failed due to unexpected error', 401, err));
    }
}